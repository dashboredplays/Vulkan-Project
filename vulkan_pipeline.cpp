...
namespace lve {
   ...
   void LvePipeline::defaultPipelineConfigInfo(PipelineConfigInfo& configInfo, uint32_t width, uint32_t height) {
      ...
      //describes transformation between pipeline output and target image
      //images are represented by pixels: top left is (0, 0), bottom right is (width, height)
      //tells how to position values to output image
      configInfo.viewport.x = 0.0f;
      configInfo.viewport.y = 0.0f;
      configInfo.viewport.width = static_cast<float>(width);
      configInfo.viewport.height = static_cast<float>(height);
      //depth range for viewport - uused to LT z values
      configInfo.viewport.minDepth = 0.0f;
      configInfo.viewport.maxDepth = 1.0f;
      
      //like viewport, but cuts out parts outside of scissor rectangle
