...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  ...
  createInstance();
  //enable validation layers, disable when shipping to production for performance
  setupDebugMessenger();
  //glfw window surface
  createSurface();
  //what device are we using? GPU here
  pickPhysicalDevice();
  //features of physical device
  createLogicalDevice();
  //command buffer allocation (memory allocation)
  createCommandPool();
}
...
void LveDevice::createBuffer(
  ...
  //allocate memory of proper size
